#!/bin/bash
#
# DAH Stage 2: Train DROID DP MLP vision on LIBERO (per-suite stage1 → per-suite stage2)
# Loads stage 1 MLP checkpoint, trains vision encoder.
#
# Usage:
#   ./scripts/train/train_dah_stage2_droid_dp_mlp_libero.sh <suite> <seed> <stage1_ckpt> [EXTRA_ARGS...]
#
# Suite options: libero_spatial, libero_object, libero_goal, libero_10
#
# Examples:
#   ./scripts/train/train_dah_stage2_droid_dp_mlp_libero.sh libero_spatial 42 data/outputs/.../checkpoints/last.ckpt
...
echo "=========================================="
echo "DAH Stage 2 — DROID DP-MLP LIBERO (per-suite)"
echo "=========================================="
echo "Suite: ${SUITE}"
echo "Seed: ${SEED}"
...
run_name="DAH_stage2_droid_dp_mlp_${SUITE}_seed${SEED}"
...
python trainer.py \
    ...
    \
    task_name="dah_stage2_droid_dp_mlp_${SUITE}" \
    libero_runner.task_suites="[${SUITE}]" \
    \
    run_dir="${run_dir}" \
    run_name="${run_name}" \
    \
    logging.project="dah_stage2_droid_dp_mlp_libero" \
    logging.name="${run_name}" \
    logging.mode="online" \
    logging.tags="[dah,stage2,droid-dp-mlp,libero,${SUITE},per-suite,language,mlp]" \
    \
    ${EXTRA_ARGS}
