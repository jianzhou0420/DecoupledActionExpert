#!/bin/bash
#
# DAH Stage 1: Train DROID DP UNet on LIBERO (per-suite, low-dim only)
#
# Usage:
#   ./scripts/train/train_dah_stage1_droid_dp_libero.sh <suite> <seed> [EXTRA_ARGS...]
#
# Suite options: libero_spatial, libero_object, libero_goal, libero_10
#
# Examples:
#   ./scripts/train/train_dah_stage1_droid_dp_libero.sh libero_spatial 42
#   ./scripts/train/train_dah_stage1_droid_dp_libero.sh libero_10 42 training.num_epochs=10
...
echo "=========================================="
echo "DAH Stage 1 — DROID DP LIBERO (per-suite)"
echo "=========================================="
echo "Suite: ${SUITE}"
echo "Seed: ${SEED}"
...
run_name="DAH_stage1_droid_dp_${SUITE}_seed${SEED}"
...
python trainer.py \
    ...
    \
    task_name="dah_stage1_droid_dp_${SUITE}" \
    \
    run_dir="${run_dir}" \
    run_name="${run_name}" \
    \
    ...
    logging.mode="online" \
    logging.tags="[dah,stage1,droid-dp,libero,${SUITE},per-suite]" \
    \
    ${EXTRA_ARGS}
