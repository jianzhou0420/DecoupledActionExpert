#!/bin/bash
#
# Train DROID Diffusion Policy (MLP) on LIBERO (local, single GPU)
#
# Usage:
#   ./scripts/train/train_droid_dp_mlp_libero.sh <suite> <seed> [EXTRA_ARGS...]
#
# Suite options: libero_spatial, libero_object, libero_goal, libero_10
#
# Examples:
#   ./scripts/train/train_droid_dp_mlp_libero.sh libero_spatial 42
#   ./scripts/train/train_droid_dp_mlp_libero.sh libero_10 42 batch_size=64
...
run_name="droid_dp_mlp_${SUITE}_seed${SEED}"
...
python trainer.py \
    ...
    \
    task_name="droid_dp_mlp_${SUITE}" \
    libero_runner.task_suites="[${SUITE}]" \
    \
    run_dir="${run_dir}" \
    run_name="${run_name}" \
    \
    logging.project="droid_dp_mlp_libero" \
    logging.name="${run_name}" \
    logging.mode="online" \
    logging.tags="[droid-dp-mlp,libero,${SUITE},lerobot,language,mlp]" \
    \
    ${EXTRA_ARGS}
