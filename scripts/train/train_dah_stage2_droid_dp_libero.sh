#!/bin/bash
#
# DAH Stage 2: Train DROID DP vision on LIBERO (per-suite stage1 → per-suite stage2)
# Loads stage 1 UNet checkpoint, freezes conv blocks, trains vision encoder.
# Auto-computes norm stats from same per-suite dataset (stats match stage 1).
#
# Usage:
#   ./scripts/train/train_dah_stage2_droid_dp_libero.sh <suite> <seed> <stage1_ckpt> [EXTRA_ARGS...]
#
# Suite options: libero_spatial, libero_object, libero_goal, libero_10
#
# Examples:
#   ./scripts/train/train_dah_stage2_droid_dp_libero.sh libero_spatial 42 data/outputs/.../checkpoints/last.ckpt
...
echo "=========================================="
echo "DAH Stage 2 — DROID DP LIBERO (per-suite)"
echo "=========================================="
echo "Suite: ${SUITE}"
echo "Seed: ${SEED}"
...
run_name="DAH_stage2_droid_dp_${SUITE}_seed${SEED}"
...
python trainer.py \
    ...
    \
    task_name="dah_stage2_droid_dp_${SUITE}" \
    ...
    logging.mode="online" \
    logging.tags="[dah,stage2,droid-dp,libero,${SUITE},per-suite,language]" \
    \
    ${EXTRA_ARGS}
