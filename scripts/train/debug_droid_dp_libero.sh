#!/bin/bash
#
# DEBUG: DROID Diffusion Policy on LIBERO
# Runs a few steps to verify the pipeline works.
#
# Usage:
#   ./scripts/train/debug_droid_dp_libero.sh [EXTRA_ARGS...]

set -e

cd "$(dirname "${BASH_SOURCE[0]}")/../.."

RUN_DIR="data/outputs/debug/debug_droid_dp_libero"
rm -rf "${RUN_DIR}"

echo "=========================================="
echo "[DEBUG] DROID DP on LIBERO"
echo "=========================================="
...
python trainer.py \
    ...
    \
    run_dir="${RUN_DIR}" \
    run_name="debug_droid_dp_libero" \
    \
    batch_size=4 \
